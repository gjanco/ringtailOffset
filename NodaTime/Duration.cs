...
namespace NodaTime
{
...

    {
        ...
        internal static readonly BigInt MinNanoseconds = (BigInt)MinDays * NanosecondsPerDay;
        internal static readonly BigInt MaxNanoseconds = (BigInt.Multiply((BigInt)MaxDays, (BigInt)NanosecondsPerDay) - 1);
        internal static readonly decimal MinDecimalNanoseconds = Convert.ToDecimal(MinNanoseconds);
        internal static readonly decimal MaxDecimalNanoseconds = Convert.ToDecimal(MaxNanoseconds);
        private static readonly double MinDoubleNanoseconds = Convert.ToDouble(MinNanoseconds);
        private static readonly double MaxDoubleNanoseconds = Convert.ToDouble(MaxNanoseconds);
        ...
        public static Duration FromNanoseconds(double nanoseconds)
        {
            return nanoseconds >= long.MinValue && nanoseconds <= long.MaxValue
                ? FromNanoseconds((long) nanoseconds) : FromNanoseconds((BigInt) nanoseconds);
        }
        ...
        /// <param name="nanoseconds">The number of nanoseconds to represent.</param>
        /// <returns>A duration with the given number of nanoseconds.</returns>
        public static Duration FromNanoseconds(BigInt nanoseconds)
        {
            ...
            long nanoOfDay = (long) (nanoseconds - ((BigInt) days) * NanosecondsPerDay);
            return new Duration(days, nanoOfDay);
        }
        ...
        [Pure]
        public BigInt ToBigIntegerNanoseconds() => IsInt64Representable ? ToInt64NanosecondsUnchecked() : ((BigInt) days) * NanosecondsPerDay + nanoOfDay;
